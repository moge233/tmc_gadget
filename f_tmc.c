...
static int32_t major = -1;
static int32_t minor = -1;
...
static struct usb_request *tmc_gadget_req_alloc(struct usb_ep *ep, unsigned int len, gfp_t gfp_flags)
{
...
static int tmc_gadget_rl_state_machine(struct tmc_device *tmc, enum tmc_gadget_rl_state_machine_event event)
{
	...
	static const char * const envp[] = {(char *)g_event_remote_local, NULL};

	kobject_uevent_env(&tmc->dev.kobj, KOBJ_CHANGE, (char **)envp);
	...
	case 0:
		if (req->actual > 0) {
			...
			if (!tmc->current_rx_bytes_remaining) {
				...
				if ((tmc->current_header.MsgID == GADGET_TMC_REQUEST_DEV_DEP_MSG_IN) || (tmc->current_header.MsgID == GADGET_TMC_REQUEST_VENDOR_SPECIFIC_IN)) {
...
static int tmc_gadget_fops_release(struct inode *inode, struct file *file)
{
	...
	tmc = (struct tmc_device *) file->private_data;
	tmc->connection_reset = true;
	...
	tmc->bulk_out_req = NULL;
	tmc->bulk_in_req = NULL;
	tmc->interrupt_req = NULL;
...
static ssize_t tmc_gadget_fops_write(struct file *file, const char __user *buf, size_t len, loff_t *offset) // @suppress("Type cannot be resolved")
{
	...
	do {
		...
		if ((tmc->current_tx_bytes_remaining == 0) && (tmc->current_header.bmTransferAttributes & TMC_XFER_TERM_CHAR_ENABLED)) {
			if (room_left)
...
static ssize_t tmc_gadget_fops_read(struct file *file, char __user *buf, size_t len, loff_t *offset) // @suppress("Type cannot be resolved")
{
	...
	if ((current_rx_bytes == 0) && !tmc->rx_complete) {
		...
		dev_dbg(&tmc->dev, "%s: line %u; rx_complete: %d, connection_reset: %d\n", __func__, __LINE__, tmc->rx_complete, tmc->connection_reset); // @suppress("Symbol is not resolved")
	}
...
static int tmc_gadget_set_interface(struct tmc_device *tmc, unsigned int interface)
{
	int result = 0;
...
static int tmc_gadget_ctrl_req_goto_local(struct usb_composite_dev *cdev, struct tmc_device *tmc, const struct usb_ctrlrequest *ctrl, struct usb_request *req)
{
	...
	if (tmc->device_capabilities.bmUSB488DeviceCapabilities & GADGET_TMC488_CAPABILITY_GOTO_LOCAL) {
		// TODO:
		static const u8 response[] = { GADGET_TMC_STATUS_SUCCESS };
...
static int tmc_gadget_function_set_alt(struct usb_function *f, unsigned int interface, unsigned int alt)
{
	...
	static const char * const envp[] = {(char *)g_event_connect, NULL};
...
static void tmc_gadget_function_suspend(struct usb_function *f)
{
	...
	static const char * const envp[] = {(char *)g_event_disconnect, NULL};
